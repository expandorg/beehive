...
func Test_makeSolutionMakerEndpoint(t *testing.T) {
	...
	resp, _ := makeSolutionMakerEndpoint(s)(cxt, sol)
	assert.Equal(t, sol, resp)
	// Error
	err := errors.New("error creating honeypot")
	s.EXPECT().
		CreateSolution(sol).
		Return(honey.Solution{}, err).
		Times(1)
	resp, e := makeSolutionMakerEndpoint(s)(cxt, sol)
